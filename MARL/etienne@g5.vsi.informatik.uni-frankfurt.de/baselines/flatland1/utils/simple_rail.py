...
def make_malfunction_training() -> Tuple[GridTransitionMap, np.array]:
    transitions = RailEnvTransitions()
    cells = transitions.transition_list
    empty = cells[0]
    right_turn_from_south = cells[8]
    ...
    simple_switch_south_right = transitions.rotate_transition(simple_switch_north_right, 180)
    rail_map = np.array(
        [[dead_end_from_east] + [simple_switch_east_right] +[horizontal_straight] * 10 + [simple_switch_west_left] + [dead_end_from_west]] +
        [[empty] + [simple_switch_south_left] + [horizontal_straight]* 10 + [simple_switch_south_right] + [empty]] +
        [[empty] + [simple_switch_south_left] + [horizontal_straight] * 10 + [simple_switch_south_right] + [empty]] +
        [[empty] + [simple_switch_south_left] + [horizontal_straight] * 10 + [simple_switch_south_right] + [empty]] +
        [[empty] + [right_turn_from_east] + [horizontal_straight] * 10 + [right_turn_from_north] + [empty]]
    , dtype=np.uint16)
    rail = GridTransitionMap(width=rail_map.shape[1],
                             height=rail_map.shape[0], transitions=transitions)
    rail.grid = rail_map

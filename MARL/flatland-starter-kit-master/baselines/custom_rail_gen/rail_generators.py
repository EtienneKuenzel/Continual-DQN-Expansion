...
from flatland.utils.simple_rail import make_custom_rail
from flatland.utils.simple_rail import make_double_track
...
def custom_rail_generator(*args, **kwargs):
    return CustomRailGen(*args, **kwargs)


class CustomRailGen(RailGen):

    def __init__(self, max_num_cities: int = 3, grid_mode: bool = False, max_rails_between_cities: int = 2,
                          max_rail_pairs_in_city: int = 3, seed=None) -> RailGenerator:
        """
        Generates railway networks with cities and inner city rails
    ...
    def generate(self, width: int, height: int, num_agents: int, num_resets: int = 0,
        ...
        # No additional inter city lines needed
        inter_city_lines = []

        # No additional rails in cities needed
        free_rails = []

        # Populate cities with custom train stations
...
def test_rail_generator(*args, **kwargs):
    return TestRailGen(*args, **kwargs)


class TestRailGen(RailGen):
    ...
    def generate(self, width: int, height: int, num_agents: int, num_resets: int = 0,
        ...
        rail_trans = RailEnvTransitions()
        rail, rail_map, optionals = make_double_track()
        grid_map = rail
        ...
        # Populate cities with custom train stations
        train_stations = [[((2, 0), 0), ((3, 0), 1)], [((2, 9), 0), ((3, 9), 1)]]

...
from flatland1.envs.fast_methods import fast_position_equal
...
class RailEnv(Environment):
    ...
    def _handle_end_reward(self, agent: EnvAgent) -> int:
        ...
        # Agents not done (arrival_time is None)
            ...
            if (agent.state.is_off_map_state()):
                reward = -1 * self.cancellation_factor * (agent.get_travel_time_on_shortest_path(self.distance_map) + self.cancellation_time_buffer)
    ...
    def get_info_dict(self):
        ...
        return info_dict
    def return_agent_pos(self):
        a = []
        for agent in self.agents:
            a.append(agent.position)
        return a
    def update_step_rewards(self, i_agent):
        """
        Update the rewards dict for agent id i_agent for every timestep
        """
        if (self.last_distance - self.agents[i_agent].get_travel_time_on_shortest_path(self.distance_map)) > 0:
            self.rewards_dict[i_agent] -= 0.05
        else:
            self.rewards_dict[i_agent] -= 1
            #vllt abhängig umso länger der umweg umso näher an 1
        self.last_distance = self.agents[i_agent].get_travel_time_on_shortest_path(self.distance_map)
    ...
    def end_of_episode_update(self, have_all_agents_ended):
        ...
        """
        if have_all_agents_ended or ((self._max_episode_steps is not None) and (self._elapsed_steps >= self._max_episode_steps)):

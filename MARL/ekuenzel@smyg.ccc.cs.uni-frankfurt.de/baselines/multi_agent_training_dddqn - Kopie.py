...
from reinforcement_learning.dddqn_policy import DDDQNPolicy, DDDQN_UCBPolicy_reset, DDDQN_UCBPolicy_timedependent, DDDQN_UCBPolicy_timedependent_reset
...
def train_agent(train_params, policy, curriculum, render=False):
    ...
    for episode_idx in range(n_episodes):
        ...
        print(episode_idx)
        if j >= 1300000:
            print("networksteps over 1  200 000")
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("-n", "--n_episodes", help="number of episodes to run", default= 10000000000, type=int)
    ...
    policies = [DDDQN_UCBPolicy_reset, DDDQN_UCBPolicy_timedependent, DDDQN_UCBPolicy_timedependent_reset]
    ...
    with open("completions_custom_dddqn.csv", "w") as outfile1:
        writer = csv.writer(outfile1)
        writer.writerow(r.keys())
        writer.writerows(zip(*r.values()))

    with open("score_custom_dddqn.csv", "w") as outfile:
